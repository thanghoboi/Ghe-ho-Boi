...
number    显示数量
start     起始数
form      模板模块 默认是library/hula_order_list.lbi
*/

function hula_user_orders($atts)
{
	...
	if ($user_id == '0'){
		...
		$orders = hula_get_user_orders($user_id, $number, $start);
	}
	$GLOBALS['smarty']->assign('orders', $orders);
	$form = (!empty($atts['form'])) ? $atts['form'] : 'library/hula_order_list.lbi';
	$val= $GLOBALS['smarty']->fetch($form);
	return $val;
}

function hula_get_user_orders($user_id, $number = 10, $start = 0) {
	$arr = array();
