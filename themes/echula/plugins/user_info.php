...
form      模板模块 默认是library/hula_history.lbi
*/

function hula_load_user_info($atts) {
	$user_id = !empty($atts['user_id']) ? $atts['user_id'] : $_SESSION['user_id'];;
	$user_info = hula_get_user_info($user_id);
	$GLOBALS['smarty']->assign('hula_user_info', $user_info);
}

function hula_get_user_info($user_id) {
	...
	$info['surplus']   = hula_price_format($row['user_money'], false);
