...
id        分类id
form      模板模块 默认是library/hula_category_tree_3.lbi
*/

function hula_category_tree_3($atts) {
	$id = !empty($atts['id']) ? intval($atts['id']) : 0;

	$GLOBALS['smarty']->assign('categories', hula_get_categories_tree_3($id));
	$GLOBALS['smarty']->assign('categories_parent', hula_get_categories_parent($id));
	$form = (!empty($atts['form'])) ? $atts['form'] : 'library/hula_category_tree_3.lbi';
	$val= $GLOBALS['smarty']->fetch($form);

	return $val;
}

function hula_get_categories_tree_3($cat_id = 0) {
	$cat_arr = array();
	if ($cat_id > 0) {
		$parent_id = $cat_id;
	...
	if ($GLOBALS['db']->getOne($sql) || $parent_id == 0) {
		...
		foreach ($res AS $row) {
			...
			if (isset($row['cat_id']) != NULL) {
				$cat_arr[$row['cat_id']]['cat_id'] = hula_get_child_tree_3($row['cat_id'], $cat_id);
			}
...
function hula_get_child_tree_3($tree_id = 0, $cat_id = 0) {
	$three_arr = array();
	if ($cat_id > 0) {
		$sql = 'SELECT parent_id FROM ' . $GLOBALS['ecs']->table('category') . " WHERE cat_id = '$cat_id'";
	...
	if ($GLOBALS['db']->getOne($sql) || $tree_id == 0) {
		...
		foreach ($res AS $row) {
			...
			if (isset($row['cat_id']) != NULL) {
				$three_arr[$row['cat_id']]['cat_id'] = hula_get_child_tree_3($row['cat_id'], $cat_id);
			}
...
function hula_get_categories_parent($cat_id = 0) {
	$cat_arr = array();
	if ($cat_id > 0) {
		$parent_id = $cat_id;
		$sql = 'SELECT parent_id FROM ' . $GLOBALS['ecs']->table('category') . " WHERE cat_id = '$cat_id'";
