...
form      模板模块 默认是library/hula_nav.lbi
*/

function hula_nav($atts) {
	...
	foreach($navlist as $k=>$v) {
		if(strlen(strtr($v['order'], '-', '')) == 1) {
			$nav[] = array(
				...
				'order'     =>  $v['order'],
				'active'    =>  $v['active'],
				'children'  =>  _hula_nav_children($navlist, $v['order']),
			);
		}
	}
	$GLOBALS['smarty']->assign('nav', $nav);
	$form = (!empty($atts['form'])) ? $atts['form'] : 'library/hula_nav.lbi';
	$val= $GLOBALS['smarty']->fetch($form);
	return $val;
}

function _hula_nav_children($navlist, $order) {
	foreach($navlist as $k=>$v) {
